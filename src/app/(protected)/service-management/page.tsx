...
const ServiceManagement = () => {
    ...
    return {
      ...
      category:
        typeof apiService.category === "object"
          ? "_id" in (apiService.category as Record<string, unknown>)
            ? (apiService.category as { _id: string })._id
            : apiService.category
          : apiService.category,
    ...
    try {
      ...
      if (paginationData) {
        setPagination({
          ...
          totalServices: paginationData.totalServices,
  ...
  }, [page, filters.category, filters.status, filters.searchQuery]);
      ...
      if (selectedServices.includes(service.id)) {
        switch (operation.type) {
          case "price":
            return {
              ...service,
              price: operation.value as number,
              updatedAt: new Date(),
            };
          case "status":
            return {
              ...service,
              isActive: operation.value as boolean,
              updatedAt: new Date(),
            };
          case "category":
  ...
  return (
    <>
      <div className="container mx-auto px-4 py-6">
        ...
        <ServiceStats
          services={services}
          totalServices={pagination.totalServices}
              ...
              <div className="space-y-4 pb-bottom-nav">
                {services.map((service) => (
                  <ServiceMobileCard
                    ...
                    onSelect={(selected) =>
                      handleSelectService(service.id, selected)
                    }
