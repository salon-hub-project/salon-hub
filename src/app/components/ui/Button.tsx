...
//         variants: {
//             variant: {
//                 // Ensure contrast by forcing white text on primary buttons
...
//                     {loading && <LoadingSpinner />}
//                     {iconName && iconPosition === 'left' && renderIcon()}
//                     {"props" in (child as React.ReactElement<any>) && (child as React.ReactElement<any>).props?.children}
//                     {iconName && iconPosition === 'right' && renderIcon()}
//                 </>
//             );
...
//                     buttonVariants({ variant, size, className }),
//                     fullWidth && "w-full",
//                     React.isValidElement(child) && "className" in (child.props as any) ? (child.props as any).className : undefined
//                 ),
//                 disabled: disabled || loading || (child.props && typeof child.props === 'object' && 'disabled' in child.props ? child.props.disabled : undefined),
//                 children: content,
//             });
...
const buttonVariants = cva(
  "inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        default:
          "bg-primary text-white hover:bg-primary hover:text-white shadow-sm hover:shadow-md",
